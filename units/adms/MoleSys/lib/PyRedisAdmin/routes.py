...
@route('/pyred_media/:file#.*#')
def media(file):
    return static_file(file, root='./lib/PyRedisAdmin/media')
...
def index():
    ...
    return template('index', all_levels_tree=all_levels_tree, config=m_config, cur_server_index=cur_server_index, media_prefix=media_prefix)
...
def view_redirect():
    ...
    if cl.exists(fullkey):
        ...
        if refmodel:
            ...
            return template('view_redirect', out_html=out_html, media_prefix=media_prefix )
    else:
        return '  This key does not exist.'
