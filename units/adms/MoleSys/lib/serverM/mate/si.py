...
class Server(object):
    ...
    def datetime(self, asstruct=False):
        '''服务器时间'''
        if not asstruct:
            return time.strftime('%Y-%m-%d %X %Z').decode("gbk")
        else:
            d = time.localtime()
            return {
                'year': d.tm_year,
    ...
    def cpustat(self, fullstat=False):
        ...
        if sys.platform == 'win32':
            import psutil
            m_info = psutil.cpu_times()
            return {'cpus':[],'total':{'all':(m_info.idle+m_info.user),'idle':m_info.idle,'used':m_info.user},'btime': time.strftime('%Y-%m-%d %X %Z',time.localtime(psutil.BOOT_TIME)).decode('gbk')}
        cpustat = {}
    ...
    def meminfo(self):
        ...
        if sys.platform == 'win32':
            class MEMORYSTATUS(Structure):
                _fields_ = [('dwLength', c_int), ('dwMemoryLoad', c_int), ('dwTotalPhys', c_int), ('dwAvailPhys', c_int), ('dwTotalPageFile', c_int),('dwAvailPageFile', c_int), ('dwTotalVirtual', c_int), ('dwAvailVirtual', c_int)]
            MEMORYSTATUS = MEMORYSTATUS()
            windll.kernel32.GlobalMemoryStatus(byref(MEMORYSTATUS))
            mem_total = MEMORYSTATUS.dwTotalPhys
            mem_free = MEMORYSTATUS.dwAvailPhys
        else:
    ...
    def mounts(self, detectdev=False):
        ...
        if sys.platform == 'win32':
            ...
            for d in devs:
                mount = {'dev': d.device, 'path': d.mountpoint, 'fstype': d.fstype}
                info = psutil.disk_usage(d.mountpoint)
                mount['total'] = b2h(info.total)
                mount['free'] = b2h(info.free)
                mount['used'] = b2h(info.used)
                mount['used_rate'] = div_percent(info.used, info.total)
                mounts.append(mount)
            return mounts
        else:
            mounts = []
    ...
    def status(self, service):
        if sys.platform == 'win32':
            ...
            try:
                ...
                return 'running|%s'%m_pid
#                p_mem = str(p.get_memory_percent())[:3]
#                p_cpu = p.get_cpu_percent(interval=1.0) 
#                return 'running|%s|%s'%(p_mem,p_cpu)
            except:
                return  'stopped'
