...
#            required = "class='wZBaseCharField required input_showDeptTree'"
        try:
            vdata = filterdata_by_user(self.choices.queryset.all().filter(device_type=5), threadlocals.get_current_user())
            need_async_nodes=[]
            btn_cancel = ""
            if not required:
...
class ZDeviceChoiceFlatWidget(ZDeviceChoiceWidget):
    flat=True

class ZDeviceMultiChoiceWidget(forms.SelectMultiple):
    flat=False
    def __init__(self, attrs={}, choices=()):
        super(ZDeviceMultiChoiceWidget, self).__init__(attrs=attrs, choices=choices)
...
class ZDeviceMultiChoiceDropDownWidget(ZDeviceMultiChoiceWidget):
    flat=False
