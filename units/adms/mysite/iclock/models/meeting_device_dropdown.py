...
def format_a_ul_cb(obj, children, data=[]):
    if children:
        ...
        return u"<li id=%s ><p%s>%s</p></li>" % (obj.pk, (obj.pk in data) and " class=s" or "", obj.ipaddress)
...
def test_dtree():
    ...
    return DeviceTree(Device.objects.all()[4989:])
...
#    return DeviceTree(vdata).html_ul_li()

 
#class ZDeviceChoiceWidgetForMeeting(forms.Select):
#    flat = False
#    def __init__(self, attrs={}, choices=()):
#        super(ZDeviceChoiceWidgetForMeeting, self).__init__(attrs=attrs, choices=choices)
...
#        try:
#            vdata = filterdata_by_user(self.choices.queryset.all().filter(device_type=1), threadlocals.get_current_user())
#            need_async_nodes=[]
#            btn_cancel = ""
#            if not required:
...
##              return u'''<input %(required)s id="dropDevice" class="wZBaseCharField input_showDeptTree" type="text" readonly="readonly" value="%(disp)s"><input type=hidden name="%(name)s" value="%(value)s"/>'
##                                <span class="btn_showDeptTree">
##                                <img onclick="render_dropdown(this, false)" src="/media/img/sug_down_on.gif" id="id_drop_dept">
##                                <div  id="show_deptment" style="display: none;">
##                                <div class="title">
##                                    %(none)s
##
...
class ZDeviceChoiceWidgetForMeeting(forms.SelectMultiple):
    flat=False
    def __init__(self, attrs={}, choices=()):
        super(ZDeviceChoiceWidgetForMeeting, self).__init__(attrs=attrs, choices=choices)
    ...
    def render(self, name, data, attrs=None, choices=()):
        ...
        try:
            ...
            html_el={
                ...
                "items": html,
                "ok":_(u"确定"),
                "selectchildren":u"%s"%_(u"全选/取消全选"),
                "need_async_nodes":",".join(need_async_nodes),
                "surl":surl
            }
            if self.flat: #平铺型
            ...
            #                u'<span class="btn_showDeptTree" style="width:200px;"><img onclick="render_dropdown(this, true)"  src="/media/img/sug_down_on.gif" id="id_drop_dept"><div id="show_deptment" style="display: none;"><div class="title"><span class="span_selectchildren displayN"></span><span onclick="javascript:$(this).parent().parent().hide();" class="close btn">%(ok)s</span></div><div id="id_dept"><ul %(attr)s>%(items)s</ul></div></div></span>')%html_el
        except:
            print_exc()
...
class ZDeviceChoiceFlatWidget(ZDeviceChoiceWidgetForMeeting):
    flat=True

class ZDeviceMultiChoiceWidgetForMeeting(forms.SelectMultiple):
    flat=False
    def __init__(self, attrs={}, choices=()):
        super(ZDeviceMultiChoiceWidgetForMeeting, self).__init__(attrs=attrs, choices=choices)
...
class ZDeviceMultiChoiceDropDownWidget(ZDeviceMultiChoiceWidgetForMeeting):
    flat=False
