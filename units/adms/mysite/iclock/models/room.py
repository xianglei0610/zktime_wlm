...
class Room(CachingModel):
    ...
    devices = DeviceManyToManyFieldKeyForMeeting(verbose_name=_(u'签到设备'),null=True,blank=False)
...
#        return  queryset.order_by('id') 
    ...
    class Admin(CachingModel.Admin):    #管理该模型
        ...
        list_display=['numberRoom','nameRoom','addressRoom','empLimit','devices'] #列表显示那些字段
...
class RoomForeignKey(models.ForeignKey):
    def __init__(self, to_field=None, **kwargs):
        super(RoomForeignKey, self).__init__(Room, to_field=to_field, **kwargs)
