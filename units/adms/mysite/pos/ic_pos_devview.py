...
#-------------------------------------验证设备记录是否已经全部上传------------------------------------------
        if request.REQUEST.has_key('type'):
            ...
            if type == "pos":
                pos_log_stamp_id = device.pos_log_stamp_id
                if pos_log_stamp_id >= devno :
                    device.pos_dev_data_status = True
                else:
                    device.pos_dev_data_status = False
            elif type == "full":
                full_log_stamp = device.full_log_stamp 
                if full_log_stamp >= devno :
                    device.pos_dev_data_status = True
                else:
                    device.pos_dev_data_status = False
            elif type == "allow":
                allow_log_stamp_id = device.allow_log_stamp_id
                if allow_log_stamp_id >= devno :
                    device.pos_dev_data_status = True
                else:
                    device.pos_dev_data_status = False
            device.save(force_update=True,log_msg=False)
...
def cdata_get_options(device):
    ...
    resp += "PosLogStamp=%s\n" % (device.pos_log_stamp or 0)
    resp += "FullLogStamp=%s\n" % (device.full_log_stamp or 0)
    resp += "AllowLogStamp=%s\n" % (device.allow_log_stamp or 0)
    #为设备最后上传消费记录的记录戳标记20120827修改
    resp += "PosLogStampId=%s\n" % (device.pos_log_stamp_id or 0)
    resp += "FullLogStampId=%s\n" % (device.full_log_stamp_id or 0)
    resp += "AllowLogStampId=%s\n" % (device.allow_log_stamp_id or 0)
    #备份记录标示
    resp += "PosBakLogStampId=%s\n" % (device.pos_log_bak_stamp_id or 0)
    resp += "FullBakLogStampId=%s\n" % (device.full_log_bak_stamp_id or 0)
    resp += "AllowBakLogStampId=%s\n" % (device.allow_log_bak_stamp_id or 0)
...
#    print u"设备主动上传的数据",s_data
    if settings.WRITEDATA_CONNECTION>0:
        ...
        try:
            ...
            try:                
                from mysite.iclock.models.model_cmmdata import pos_gen_device_cmmdata
                from mysite.pos.pos_ic.ic_sync_store import save_att_file
                if POS_IC_ADMS_MODEL:
                    obj = save_att_file(device,s_data)
                else:
