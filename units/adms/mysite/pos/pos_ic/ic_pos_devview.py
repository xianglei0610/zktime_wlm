...
from mysite.pos.pos_constant import POS_IC_ADMS_MODEL,POS_DEAL_BAT_SIZE,ONLINE_ALLOWANCE,POS_DEBUG
...
from mysite.pos.pos_utils import delete_model_cache
...
db_contion_error = False # 数据库连接是否异常
...
def pos_business(request,device):
    ...
    if ONLINE_ALLOWANCE and rep_type == '10': #在线补贴，支持一个月多次补贴
        return online_allowance(request,device)
    else:
        resp = ""
        if card:
            try:
...
#在线补贴定制，支持一个月多次补贴
def online_allowance(request,device):
    ...
    resp = ""
    sql = "select  batch,money,valid_date from pos_allowance where sys_card_no = %s and batch > %s and is_pass=1 "%(card,batch)
    allowance_list = p_query(sql)
    try:
        obj_card = IssueCard.objects.get(sys_card_no = card)
    except:
        obj_card = None
    if obj_card:
        if allowance_list:
            resp += "RET=OK\n"
            for obj in allowance_list:
                allowdata = ((obj[2].year-2000)*12+obj[2].month)*31+obj[2].day
                resp += "SysID=%s\tCardNo=%s\tBatch=%s\tMoney=%s\tallowDate=%s\n" %(card,card,obj[0],int(obj[1]*100),allowdata)
        else:
            resp += "RET=NO\tRESON=%s" % _(u"无补贴记录")
    else:
        resp += "RET=NO\tRESON=%s" % _(u"卡号不存在")
...
def commit_log_(sql,stamp_name):
    sql_exc=""
            ...
            try:
                if settings.DATABASE_ENGINE == "sqlserver_ado":
                    if data[0] == 'NULL':
                        ...
                        sql_s = check_factor + error_log_statement
                        operation = sql_s % tuple(data)
                        batch_sql +=operation
                    else:
                        check_factor = check_repeated_data(stamp_name,data)
                ...
                import traceback;traceback.print_exc()
#        print "sql_s===================",batch_sql
        if settings.DATABASE_ENGINE == "sqlserver_ado":
            try:
                res = p_execute(batch_sql)
            except:
                import traceback;traceback.print_exc()
#            print "dbutil__res++++++++++++++",res,len(sql)
            if res is None:#连接池接口出现异常情况（执行sql报错）
                raise DatabaseError
            if res == -2:#数据库连接断开
                db_contion_error = True
                raise DatabaseError
    elif type(sql)==type((0,)):
        if settings.DATABASE_ENGINE == "sqlserver_ado":
            if sql[0] == 'NULL':
                check_factor = check_repeated_data(stamp_name,sql)
                sql_s = check_factor + error_log_statement
                operation = sql_s % tuple(sql)
                res = p_execute(operation)
                if res is None:#连接池接口出现异常情况（执行sql报错）
                    raise DatabaseError
                if res == -2:#数据库连接断开
                    db_contion_error = True
...
def commit_log(sql,stamp_name):
    try:
        commit_log_(sql,stamp_name)
    except DatabaseError:
        raise DatabaseError
    except:
        print "----------------------------------------------commit_log---------------------------------------%s"%datetime.datetime.now()
        import traceback;traceback.print_exc()
...
pos_log_statement="""insert into pos_icconsumerlist (user_id,user_pin, user_name, dept_id, card, sys_card_no, dev_sn, card_serial_num,dev_serial_num,pos_time,convey_time,\
type_name,money,balance,pos_model,dining_id,meal_id,meal_data,create_operator,log_flag)
values('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%s',%s, '%s','%s','%s');"""

#卡表中找不到卡账号的记录
error_log_statement="""insert into pos_icerrorlog (user_id,user_pin, user_name, dept, card, sys_card_no, dev_sn, card_serial_num,dev_serial_num,pos_time,convey_time,\
type_name,money,balance,pos_model,dining_id,meal_id,meal_data,create_operator,log_flag)
values('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%s',%s, '%s','%s','%s');"""

#消费记录备份
pos_log_bak_statement="""insert into pos_icconsumerlistbak (user_id,user_pin, user_name, dept_id, card, sys_card_no, dev_sn, card_serial_num,dev_serial_num,pos_time,convey_time,\
type_name,money,balance,pos_model,dining_id,meal_id,meal_data,create_operator,log_flag)
values('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s','%s', '%s', '%s', '%s', '%s', '%s', '%s',%s, '%s','%s','%s');"""

#根据卡账号获取人员信息
def card_to_userinfo(user_pk):
...
def line_to_pos_bak_log(device, line, stamp_name,event=True):
    ...
    try:
        card = IssueCard.all_objects.get(sys_card_no=flds[1],card_privage = POS_CARD)
#        if card.failuredate is None or card.failuredate >= pos_time:#过滤掉无卡退卡后的卡所产生的消费记录
        if flds[6] == '9':
            ...
            pos_money = Decimal(flds[3])/100
        if card.create_time > pos_time:#当消费卡发卡日期 大于 消费记录的消费日期的时候，系统认定为异常记录
            log_flag = 5 #异常记录
        else:
            log_flag = 1
        if flds[7] == '0':
            ...
            meal_id = flds[7]
        emp_obj = card_to_userinfo(card.UserID_id)
        user_pin = emp_obj.PIN
        user_name = emp_obj.EName
        user_dept_pk = emp_obj.DeptID_id
#            sql = (card.UserID_id,card.UserID.PIN, card.UserID.EName,card.UserID.DeptID.pk,card.cardno,flds[1],device.sn,flds[5],flds[9],flds[2],now,logtype,pos_money,Decimal(flds[4])/100, flds[6],device.dining.pk,meal_id,flds[2], flds[10],1)
        sql = (card.UserID_id,user_pin,user_name,user_dept_pk,card.cardno,flds[1],device.sn,flds[5],flds[9],flds[2],now,logtype,pos_money,Decimal(flds[4])/100, flds[6],device.dining.pk,meal_id,flds[2], flds[10],log_flag)
        return sql
...
#        import traceback; traceback.print_exc();
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_pos_bak_log>%s",datetime.datetime.now()
        return None
...
#            user_dept_pk = get_dept(card.UserID_id).id
            
            sql = (card.UserID_id,user_pin, user_name,user_dept_pk,card.cardno,flds[1],device.sn,flds[5],flds[9],flds[2],now,logtype,pos_money,Decimal(flds[4])/100, flds[6],device.dining.pk,meal_id,flds[2], flds[10],log_flag)
        else:#当前数据库卡表中找不到的卡所产生的消费记录
            log_flag = 6  #卡账号异常记录
            sql = ('NULL','NULL', 'NULL','NULL','NULL',flds[1],device.sn,flds[5],flds[9],flds[2],now,logtype,pos_money,Decimal(flds[4])/100, flds[6],device.dining.pk,meal_id,flds[2], flds[10],log_flag)
#            print "sql================",sql
        return sql
    except(OperationalError,InternalError):
        global db_contion_error
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_pos_log>%s",datetime.datetime.now()
        return None


full_log_statement="""insert into pos_carcashsz (user_id, card, sys_card_no, sn_name, cardserial,serialnum,checktime,convey_time,\
type_id,money,blance,hide_column,create_operator,log_flag,status)
values('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s', '%s', '%s', '%s', '%s','%s','%s');"""

full_log_bak_statement="""insert into pos_carcashszbak (user_pin, user_name, DeptName,physical_card_no,sys_card_no, sn_name, cardserial,serialnum,checktime,convey_time,\
...
#        if card.failuredate is None or card.failuredate >= full_time:#过滤掉无卡退卡后的卡所产生的充值记录
        ...
        if card.create_time > full_time:#当消费卡发卡日期 大于 充值记录的充值日期的时候，系统认定为异常记录
            log_flag = 5 #异常记录
        else:
            log_flag = 1
        sql = (card.UserID_id,card.cardno,flds[1],device.sn,flds[2],flds[7],flds[3],now,logtype,Decimal(flds[4])/100,Decimal(flds[5])/100, logtype,flds[8],log_flag,0)
        return sql
    except ObjectDoesNotExist:
        return None
    except(OperationalError,InternalError):
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_full_log>%s",datetime.datetime.now()
        return None
...
#        if card.failuredate is None or card.failuredate >= full_time:#过滤掉无卡退卡后的卡所产生的充值记录
        ...
        sql = (user_pin,user_name,user_dept_name,card.cardno,flds[1],device.sn,flds[2],flds[7],flds[3],now,Decimal(flds[4])/100,Decimal(flds[5])/100, logtype,flds[8],log_flag,0)
        return sql
    except ObjectDoesNotExist:
        return None
    except(OperationalError,InternalError):
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_full_bak_log>%s",datetime.datetime.now()
        return None
    


allow_log_statement="""insert into pos_carcashsz (user_id, card, sys_card_no, sn_name, cardserial,serialnum,checktime,convey_time,\
type_id,money,blance,hide_column,allow_type,allow_batch,allow_base_batch,log_flag,status)
values('%s', '%s', '%s', '%s', '%s', '%s', '%s','%s','%s', '%s', '%s', '%s', '%s','%s', '%s', '%s','%s');"""

allow_log_bak_statement="""insert into pos_carcashszbak (user_pin, user_name, DeptName,physical_card_no,sys_card_no, sn_name, cardserial,serialnum,checktime,convey_time,\
...
#        if card.failuredate is None or card.failuredate >= allow_time:#过滤掉无卡退卡后的卡所产生的补贴记录
        if card.create_time > allow_time:#当消费卡发卡日期 大于 补贴记录的补贴领取日期的时候，系统认定为异常记录
            log_flag = 5 #异常记录
        else:
            log_flag = 1
        logtype = LOGTYPE[stamp_name]
        ...
        sql = (user_pin,user_name,user_dept_name,card.cardno,flds[1],device.sn,flds[2],flds[9],flds[4],now,Decimal(flds[5])/100,Decimal(flds[6])/100, logtype,flds[7],flds[3],flds[8],log_flag,0)
        return sql
    except ObjectDoesNotExist:
#        import traceback; traceback.print_exc();
        return None
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_allow_bak_log>%s",datetime.datetime.now()
        return None
...
#        if card.failuredate is None or card.failuredate >= allow_time:#过滤掉无卡退卡后的卡所产生的补贴记录
        logtype = LOGTYPE[stamp_name]
        sql = (card.UserID_id,card.cardno,flds[1],device.sn,flds[2],flds[9],flds[4],now,logtype,Decimal(flds[5])/100,Decimal(flds[6])/100, logtype,flds[7],flds[3],flds[8],log_flag,0)
        return sql
    except ObjectDoesNotExist:
        return None
    except(OperationalError,InternalError):
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_allow_log>%s",datetime.datetime.now()
        return None
    
    
store_detail_statement="""insert into pos_storedetail (list_code_id, dev_sn,dev_serial_num, store_code, money, RecSum,convey_time)values('%s', '%s', '%s','%s', '%s', '%s', '%s');"""
...
#        import traceback; traceback.print_exc();
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_store_detail>%s",datetime.datetime.now()
        return None
    
    
key_detail_statement="""insert into pos_keydetail (list_code_id,dev_sn, dev_serial_num, key_code, money, RecSum,convey_time)values('%s', '%s','%s', '%s', '%s', '%s', '%s');"""
...
#        import traceback; traceback.print_exc();
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_key_detail>%s",datetime.datetime.now()
        return None
    

time_detail_statement="""insert into pos_timedetail (list_code_id,dev_sn, dev_serial_num, begin_time, begin_money, end_time,end_money,convey_time)values('%s','%s','%s', '%s', '%s','%s','%s','%s');"""
...
#        import traceback; traceback.print_exc();
        ...
        db_contion_error = True
        conn.close()
        print u"数据库连接失败=======line_to_time_detail>%s",datetime.datetime.now()
        return None
...
#        if data[0] == 'NULL':
        sql = """
                    if  not exists(
                    select dev_sn from (
                    select dev_sn,sys_card_no,money,card_serial_num,dev_serial_num,pos_time from pos_icerrorlog where  dev_serial_num is not null
                    union 
                    select dev_sn,sys_card_no,money,card_serial_num,dev_serial_num,pos_time from pos_icconsumerlist where dev_serial_num is not null
                    ) as b %(where)s 
            """
#            SELECT_ICCONSUMERLIST = "if  not exists(select user_id from pos_icerrorlog where (dev_sn='%s' and sys_card_no='%s' and money='%s' and card_serial_num='%s'and dev_serial_num='%s' and pos_time ='%s'))" % (data[6],data[5],data[12],data[7],data[8],data[9])
            
        SELECT_ICCONSUMERLIST = sql%({'where':" where (dev_sn='%s' and sys_card_no='%s' and money='%s' and card_serial_num='%s'and dev_serial_num='%s' and pos_time ='%s'))"% (data[6],data[5],data[12],data[7],data[8],data[9])})
#        else:
...
def cdata_post_trans(device, raw_data,stamp_name, head=None,old_head=None):
    '''
        2012-04-24 日重新修改消费记录解析方案：基于python--dbutils 数据库连接池方案，当前方案只考虑sql_server
    '''
#    cursor = conn.cursor()
    global db_contion_error
    db_contion_error = False # 数据库连接是否异常
            ...
            if log:
                ...
                if len(cacheLines) >= commitLineCount: #达到一定的行就提交一次
                    try:
                        ...
                        print "------------------------cdata_post_trans_error--------------------------------%s"%datetime.datetime.now()
                        import traceback; traceback.print_exc();
                        errorLines += cacheLines
                    cacheLines = []
                    sqls = []
...
#    print "card_dict",card_dict
    ...
    if errorLines:
        print "errorLineserrorLineserrorLineserrorLineserrorLineserrorLineserrorLines"
        #ErrNo:22 (invalid argument)的错误处理。
        #出错的非重复记录，写入错误日志文件mysite/files/zkpos/device.sn/error
        elinedata=[]
...
#    super(Device, device).save(force_update=True,log_msg=False)
    return (okc, len(errorLines), dlogObj,db_contion_error)
...
def commit_omSql(sql,action=True):
    try:
        cur = conn.cursor()            
        cur.execute(sql)
        if action:
...
#        print _(u'更新卡余额'),card_dict
        if more_data:
            ...
            if stamp_name == 'pos_log_stamp' :#上传消费记录 
                obj_con = p_query_one(select_issuecard%dit)
                if obj_con and obj_con[0]:
                    max_con_serial = obj_con[0]
                else:
                    max_con_serial = 0
                if int(max_serial) > int(max_con_serial) :
                    try:
                        update_sql = "update personnel_issuecard set blance = %s,card_serial_num = %s where sys_card_no = %s and card_privage = %s " %(blance,max_serial,dit,POS_CARD)
                        res = p_execute(update_sql)
                        if res:
                            obj_card =IssueCard.all_objects.get(sys_card_no = dit)#手动更新缓存
                            delete_model_cache(obj_card)
#                        obj_card.blance = blance
#                        obj_card.card_serial_num = max_serial
#                        models.Model.save(obj_card)
...
#                    obj_card.save(force_update=True,log_msg=False)
            ...
            if stamp_name in ['full_log_stamp','allow_log_stamp','pos_log_stamp']:#上传单条记录
                try:
                    obj_card = IssueCard.all_objects.get(sys_card_no = dit)
                    obj_card.blance = blance
                    models.Model.save(obj_card,force_update=True)
                except:
                    pass
